...
/// <summary>
/// Runs post-boss rumble and a bounded stream of escape enemies after the exit is revealed.
/// Spawn candidates come from an assigned tilemap or a pre-sampled region; enemy scaling follows the final floor.
/// </summary>
public sealed class BossEscapeSequenceManager : MonoBehaviour
{
    ...
    /// <summary>
    /// Whether either supported source can supply escape spawn positions.
    /// </summary>
    private bool HasSpawnArea =>
        enemySpawnTilemap != null || spawnRegion != null;

    /// <summary>
    /// Adds the first reveal rumble and its pause before the camera returns to the player.
    /// </summary>
    ...
    /// <summary>
    /// Starts rumble and eligible spawn loops once, after the boss outro restores player control.
    /// </summary>
    ...
    /// <summary>
    /// Stops future rumble and spawning while leaving already spawned enemies in the scene.
    /// </summary>
    ...
    /// <summary>
    /// Stops owned escape coroutines when their scene object is destroyed.
    /// </summary>
    ...
    /// <summary>
    /// Uses the authored camera or a scene fallback for rumble.
    /// </summary>
    ...
    /// <summary>
    /// Uses the configured distance-reference transform, otherwise resolves the tagged player.
    /// </summary>
    ...
    /// <summary>
    /// Uses the mean authored cell size to interpret spawn-distance tuning, or one unit for collider regions.
    /// </summary>
    ...
    /// <summary>
    /// Caches occupied tile centers or samples the collider's bounding box once at escape startup.
    /// </summary>
    ...
    /// <summary>
    /// Converts world distance to authored tile units while guarding a zero cell size.
    /// </summary>
    ...
    /// <summary>
    /// Chooses player-ring or exit-ring candidates, periodically favoring the exit once five enemies have spawned.
    /// </summary>
    private Vector2? PickSpawnWorldPosition()
    {
        ...
        List<Vector2> playerRing = new List<Vector2>();
        List<Vector2> teleRing = new List<Vector2>();
    ...
    /// <summary>
    /// Enforces live and total limits, chooses a valid position and prefab, then scales and tracks the new enemy.
    /// </summary>
    ...
    /// <summary>
    /// Applies at least final-normal-floor scaling plus the escape health and damage bonuses.
    /// </summary>
    ...
    /// <summary>
    /// Repeats camera rumble at randomized scaled-time intervals while escape remains active.
    /// </summary>
    ...
    /// <summary>
    /// Creates an initial burst, then bounded periodic spawns; twelve failed attempts end the loop.
    /// </summary>
    private IEnumerator EscapeSpawnLoop()
    {
        List<(GameObject prefab, float weight)> pool = BuildWeightedPool();
        if (pool.Count == 0)
        {
            _spawnLoop = null;
            yield break;
    ...
    /// <summary>
    /// Removes destroyed instances before checking the concurrent enemy limit.
    /// </summary>
    ...
    /// <summary>
    /// Filters missing prefabs and nonpositive weights out of the authored enemy pool.
    /// </summary>
    private static List<(GameObject prefab, float weight)> BuildWeightedPoolFromInspector(List<WeightedEnemyPrefab> list)
    {
        List<(GameObject, float)> pool = new List<(GameObject, float)>();
        if (list == null)
            return pool;
        foreach (WeightedEnemyPrefab entry in list)
        {
    ...
    /// <summary>
    /// Builds the current encounter's valid weighted enemy pool.
    /// </summary>
    ...
    /// <summary>
    /// Draws from a nonempty weighted pool with a final-entry fallback for floating-point boundaries.
    /// </summary>
    private static GameObject GetRandomPrefab(List<(GameObject prefab, float weight)> pool)
    {
        float totalWeight = 0f;
        foreach ((GameObject prefab, float weight) entry in pool)
