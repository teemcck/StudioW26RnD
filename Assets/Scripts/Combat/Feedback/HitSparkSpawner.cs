...
// Runs before default-order scripts so Instance is assigned before EnemyBase / weapons request sparks on the first hit.
...
public sealed class HitSparkSpawner : MonoBehaviour
{
    ...
    private void OnEnemyDamaged(EnemyDamagedEvent evt)
    {
        ...
        HitSparkCategory category = Classify(evt);
    ...
    public void Spawn(Vector2 position, Color color, HitSparkCategory category)
    {
        ...
        GameObject go = Instantiate(hitSparkPrefab, new Vector3(position.x, position.y, 0f), Quaternion.identity);
        ...
        HitSparkAnim anim = go.AddComponent<HitSparkAnim>();
