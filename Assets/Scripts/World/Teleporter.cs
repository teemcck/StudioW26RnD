...
public class Teleporter : MonoBehaviour
{
    ...
    private IEnumerator PerformChunkTransition(Transform player, Vector3 targetPos)
    {
        ...
        ChunkTransitionOverlay overlay = ChunkTransitionOverlay.Instance;
        if (overlay != null)
        {
    ...
    private static void MovePlayerWithCameraSnap(Transform player, Vector3 targetPos, float graceSeconds)
    {
        ...
        if (follow != null)
        {
            ...
            TrackerSettings s = follow.TrackerSettings;
        ...
        PlayerHealth health = player.GetComponent<PlayerHealth>() ?? player.GetComponentInChildren<PlayerHealth>();
        if (health != null)
            health.BeginTeleporterArrivalGrace(graceSeconds);

        if (hadDamping)
        {
            RestoreDampingNextFrame runner = cam.gameObject.AddComponent<RestoreDampingNextFrame>();
...
internal sealed class RestoreDampingNextFrame : MonoBehaviour
{
    ...
    private IEnumerator Start()
    {
        ...
        if (_follow != null)
        {
            TrackerSettings s = _follow.TrackerSettings;
