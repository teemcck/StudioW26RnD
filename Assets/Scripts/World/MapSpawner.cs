...
public class MapSpawner : MonoBehaviour
{
    ...
    public List<GameObject> GenerateRandomSequence(int difficulty, int numChunks, WorldBand band)
    {
        ...
        List<(GameObject prefab, float weight)> weightedPool = BuildWeightedPool(band);
        ...
        for (int i = 0; i < numChunks; i++)
        {
            ...
            {
                List<GameObject> spawnedHere = spawn.SpawnEnemiesFromTileLayer(weightedPool, fillFraction);
    ...
    {
        Teleporter tp = lastChunk.GetComponentInChildren<Teleporter>();
        ...
        SpawnEnemies[] spawners = lastChunk.GetComponentsInChildren<SpawnEnemies>(true);
        ...
        GameObject go = Instantiate(prefab, spawnWorld, Quaternion.identity, lastChunk.transform);
    ...
    private static Vector3 FindEliteSpawnOnGroundNear(Vector2 teleporterWorld, float chunkZ, int indexInGroup)
    {
        ...
        for (int ring = 0; ring < 14; ring++)
        {
            ...
            for (int s = 0; s < spokes; s++)
            {
                ...
                RaycastHit2D hit = Physics2D.Raycast(origin, Vector2.down, 28f, mask);
        ...
        RaycastHit2D lastHit = Physics2D.Raycast(up, Vector2.down, 28f, mask);
    ...
    List<(GameObject prefab, float weight)> BuildWeightedPool(WorldBand band)
    {
        ...
        List<WeightedEnemyPrefab> sourcePool = GetEnemyPoolForBand(band);
        if (sourcePool == null)
            return list;

        foreach (WeightedEnemyPrefab entry in sourcePool)
        {
