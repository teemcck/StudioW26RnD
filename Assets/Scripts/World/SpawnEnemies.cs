...
public class SpawnEnemies : MonoBehaviour
{
    ...
    public List<GameObject> SpawnEnemiesFromTileLayer(List<(GameObject prefab, float weight)> enemyPool, float fillFraction)
    {
        ...
        TeleportEntry entry = GetComponentInParent<TeleportEntry>() ?? GetComponentInChildren<TeleportEntry>();
        if (entry == null)
        {
        ...
        for (int i = 0; i < availableTiles.Count && placed < targetCount; i++)
        {
            ...
            GameObject go = Instantiate(prefab, spawnPos, Quaternion.identity, parentTransform);
            spawned.Add(go);
    ...
    private bool IsSpawnPositionOverGround(Vector3 worldPos)
    {
        ...
        RaycastHit2D hit = Physics2D.Raycast(origin, Vector2.down, groundProbeUp + groundProbeDown, groundCheckMask);
    ...
    private GameObject GetRandomEnemyType(List<(GameObject prefab, float weight)> enemyPool)
    {
        ...
        foreach ((GameObject prefab, float weight) entry in enemyPool)
