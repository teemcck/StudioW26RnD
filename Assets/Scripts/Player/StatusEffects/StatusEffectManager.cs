...
{
    private Dictionary<StatusEffectId, StatusEffect> _activeEffects = new();
    ...
    /// </summary>
    public void Apply(StatusEffectId effectId, float duration, int maxStacks = 1, bool isPermanent = false)
    {
        ...
        {
            StatusEffect effect = new StatusEffect(effectId, duration, maxStacks, isPermanent);
    ...
    /// </summary>
    public void Remove(StatusEffectId effectId)
    {
        if (_activeEffects.Remove(effectId))
            RefreshStatBackedEffects();
    }

    public void SetPermanentStacks(StatusEffectId effectId, int stacks)
    {
        if (stacks <= 0)
        {
            Remove(effectId);
        ...
        {
            StatusEffect effect = new StatusEffect(effectId, 0f, stacks, isPermanent: true)
            {
    ...
    /// </summary>
    public bool Has(StatusEffectId effectId)
    {
    ...
    {
        return _activeEffects.TryGetValue(effectId, out StatusEffect effect) ? effect.currentStacks : 0;
    }
    ...
    {
        TickPoison();
        List<StatusEffectId> expiredEffects = new List<StatusEffectId>();
        
        foreach (KeyValuePair<StatusEffectId, StatusEffect> kvp in _activeEffects)
        {
        ...
        foreach (StatusEffectId id in expiredEffects)
    ...
    {
        int swiftnessStacks = Mathf.Min(3, GetStackCount(StatusEffectId.Swiftness));
        int confusionStacks = Mathf.Min(2, GetStackCount(StatusEffectId.Confusion));
