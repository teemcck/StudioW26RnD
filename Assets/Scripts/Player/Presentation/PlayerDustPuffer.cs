...
public sealed class PlayerDustPuffer : MonoBehaviour
{
    ...
    private void SpawnPuff(float alphaScale)
    {
        ...
        GameObject go = Instantiate(dustPuffPrefab, new Vector3(transform.position.x, transform.position.y - 0.25f, 0f), Quaternion.identity);
        ApplyAlphaAndFade(go, alphaScale, Vector2.zero);
    }

    private void SpawnSkid(Vector2 direction)
    {
        GameObject prefab = dashSkidPuffPrefab != null ? dashSkidPuffPrefab : dustPuffPrefab;
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            GameObject go = Instantiate(prefab, new Vector3(pos.x, pos.y - 0.25f, 0f), Quaternion.identity);
    ...
    {
        SpriteRenderer sr = go.GetComponent<SpriteRenderer>() ?? go.GetComponentInChildren<SpriteRenderer>();
        if (sr != null)
        {
            Color c = dustColor;
            c.a *= alphaScale;
        ...
        DustPuffFade fade = go.GetComponent<DustPuffFade>() ?? go.AddComponent<DustPuffFade>();
        fade.Init(sr, 0.35f);
