...
public class PlayerUpgradeManager : MonoBehaviour
{
    ...
    public void NotifyAttackPerformed(AttackKind attackKind, AttackDamageSnapshot snapshot)
    {
        if (_whirlwindCount > 0 && _statusEffects != null)
            _statusEffects.Apply(StatusEffectId.Swiftness, 1f, 3);
    ...
    private void SyncSpecialStates()
    {
        ...
        if (_unsteadyPoisonStacks > 0)
            _statusEffects.SetPermanentStacks(StatusEffectId.Poison, _unsteadyPoisonStacks);
        else
            _statusEffects.Remove(StatusEffectId.Poison);
    }
    ...
    private void OnEnemyDamaged(EnemyDamagedEvent evt)
    {
        ...
        EnemyStatusEffectController enemyStatuses = evt.Enemy ? evt.Enemy.StatusEffects : null;
        if (enemyStatuses == null)
            return;

        bool makePermanent = ShouldInflictedStatusesBePermanent;
        if (_poisonOnHitDuration > 0f)
            enemyStatuses.Apply(StatusEffectId.Poison, _poisonOnHitDuration, int.MaxValue, makePermanent, evt.Context);
        if (_confusionOnHitDuration > 0f)
            enemyStatuses.Apply(StatusEffectId.Confusion, _confusionOnHitDuration, 3, makePermanent, evt.Context);
        if (_frailtyOnHitDuration > 0f)
            enemyStatuses.Apply(StatusEffectId.Frailty, _frailtyOnHitDuration, 1, makePermanent, evt.Context);
