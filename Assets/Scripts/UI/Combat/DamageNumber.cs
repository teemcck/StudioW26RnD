...
// Runs before default-order scripts so this subscribes to EnemyDamagedEvent (OnEnable) ahead of any enemy or
// weapon that could raise it in the scene's first frame; Instance is exposed for direct Spawn() calls.
    ...
    public void Spawn(Vector2 worldPos, float damage, bool isCrit)
    {
        ...
        if (isCrit)
        {
            ...
            if (HitstopController.Instance != null)
                HitstopController.Instance.Freeze(0.07f, priority: 2);
        ...
        DamageNumberAnim anim = go.AddComponent<DamageNumberAnim>();
