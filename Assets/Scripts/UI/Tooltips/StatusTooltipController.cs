...
// Instance must exist before TooltipLinkRelay (default order) forwards hover/click events to it from OnEnable.
...
public sealed class StatusTooltipController : MonoBehaviour
{
    ...
    public void Show(string key, Vector2 screenPos)
    {
        ...
        if (!StatusEffectCatalog.TryGet(key, out StatusEffectCatalog.Entry entry))
        {
            Hide();
            return;
        }
