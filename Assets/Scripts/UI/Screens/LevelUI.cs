...
public class LevelUI : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        previewBeginButton.onClick.AddListener(OnPreviewBeginClicked);
        summaryContinueButton.onClick.AddListener(OnSummaryContinueClicked);
        xpBarContinueButton.onClick.AddListener(OnXpBarContinueClicked);
    }

    private void OnDestroy()
    {
        if (previewBeginButton != null) previewBeginButton.onClick.RemoveListener(OnPreviewBeginClicked);
        if (summaryContinueButton != null) summaryContinueButton.onClick.RemoveListener(OnSummaryContinueClicked);
        if (xpBarContinueButton != null) xpBarContinueButton.onClick.RemoveListener(OnXpBarContinueClicked);
    }

    private void OnPreviewBeginClicked()
    {
    ...
    private void OnXpBarContinueClicked()
    {
        AudioManager.Instance?.PlayUiButton();
        xpBarPanel.SetActive(false);
    ...
    private void RestoreSummaryPresentationTransforms()
    {
        ...
        if (summaryPanel != null && summaryPanel.TryGetComponent<RectTransform>(out RectTransform panelRt))
    ...
    {
        CacheSummaryLayout();
        Vector2 offset = new Vector2(-Mathf.Abs(summarySlamOffscreenX), 0f);
        ...
        if (timeRt != null)
        {
            ...
            timeRt.anchoredPosition = _timeAnchoredRest + offset;
        }
        if (xpRt != null)
        {
            summaryXPText.text = string.Empty;
            xpRt.anchoredPosition = _xpAnchoredRest + offset;
        }
        if (killsRt != null)
        {
            summaryKillsText.text = FormatEnemyXpBlock(killed, avoided, 0, 0);
            killsRt.anchoredPosition = _killsAnchoredRest + offset;
        ...
        if (killsRt != null)
        {
            yield return SlamRect(killsRt, _killsAnchoredRest + offset, _killsAnchoredRest, summarySlamDuration);
        ...
        if (timeRt != null)
        {
            ...
            yield return SlamRect(timeRt, _timeAnchoredRest + offset, _timeAnchoredRest, summarySlamDuration);
            AudioManager.Instance?.PlayXpSummaryBlockLand(1.5f);
        ...
        if (xpRt != null)
        {
            ...
            yield return SlamRect(xpRt, _xpAnchoredRest + offset, _xpAnchoredRest, summarySlamDuration);
            AudioManager.Instance?.PlayXpSummaryBlockLand(1.5f);
    ...
    private IEnumerator AnimateDifficultyRoll(int finalDifficulty)
    {
        float elapsed = 0f;
    ...
    private IEnumerator AnimateXPBar(int startXP, int targetXP, int xpPerLevel)
    {
        ...
        SetXpBarFill((float)finalDisplayXP / xpPerLevel);
