...
public sealed class AppliedUpgradeStripUI : MonoBehaviour
{
    ...
    public void EnsureStripRootUnderCanvas(Canvas hostCanvas)
    {
        ...
        GameObject rootGo = new GameObject("StripRoot", typeof(RectTransform));
    ...
    public void OpenOverflowAllListPanel()
    {
        ...
        Canvas canvas = stripRoot.GetComponentInParent<Canvas>();
        if (canvas == null)
            return;

        AppliedUpgradesOverflowListPanel panel = AppliedUpgradesOverflowListPanel.EnsureOnCanvas(canvas);
    ...
    private void HideOverflowListPanelIfOpen()
    {
        ...
        if (canvas == null)
            return;
        AppliedUpgradesOverflowListPanel panel = canvas.GetComponentInChildren<AppliedUpgradesOverflowListPanel>(true);
    ...
    {
        GameObject iconGo = Instantiate(iconPrefab, stripRoot);
        ...
        CardHover hover = iconGo.GetComponent<CardHover>() ?? iconGo.AddComponent<CardHover>();
        hover.Configure(display, stackCount);
        hover.ConfigureDetailPlacement(CardHover.DetailAnchor.BelowAnchor, compact: true);

        Transform badge = iconGo.transform.Find("Badge");
        if (badge != null)
        {
            badge.gameObject.SetActive(stackCount >= 2);
            TMP_Text badgeText = badge.Find("Text")?.GetComponent<TMP_Text>();
            if (badgeText != null)
    ...
    {
        GameObject chipGo = Instantiate(overflowChipPrefab, stripRoot);
        ...
        TMP_Text text = chipGo.transform.Find("Text")?.GetComponent<TMP_Text>();
        if (text != null)
        ...
        OverflowChipOpenAppliedList opener = chipGo.GetComponent<OverflowChipOpenAppliedList>();
        if (opener == null)
    ...
    public sealed class OverflowChipOpenAppliedList : MonoBehaviour, IPointerClickHandler
    {
        ...
        public void OnPointerClick(PointerEventData eventData)
        {
            ...
            Canvas canvas = _host.StripRoot.GetComponentInParent<Canvas>();
            if (canvas == null)
                return;
