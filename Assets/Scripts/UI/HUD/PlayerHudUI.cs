...
public sealed class PlayerHudUI : MonoBehaviour
{
    ...
    {
        foreach (PlayerHudUI hud in Object.FindObjectsByType<PlayerHudUI>(FindObjectsInactive.Include, FindObjectsSortMode.None))
    ...
    private void ApplyAlmostLevelPulse(float xpNormalized)
    {
        ...
        UnityEngine.UI.Image img = xpFillRect.GetComponent<UnityEngine.UI.Image>();
        if (img == null)
            return;
    ...
    {
        if (GameplayHandler.Instance != null)
            return Mathf.Max(1, GameplayHandler.Instance.XPPerLevel);

        if (RunStatsTracker.Instance != null && RunStatsTracker.Instance.XpPerLevelHudOverride > 0)
            return RunStatsTracker.Instance.XpPerLevelHudOverride;

        return Mathf.Max(1, GameplayHandler.LastPublishedXpPerLevel);
    }
    ...
    private void FixHudCanvasScaleIfBroken()
    {
        ...
        RectTransform rt = hudCanvas.transform as RectTransform;
        if (rt == null)
            return;
