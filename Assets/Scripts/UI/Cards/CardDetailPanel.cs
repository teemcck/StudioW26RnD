...
// Instance must exist before CardHover and AppliedUpgradesOverflowListPanel (default order) call Show/Hide from their OnEnable/OnDisable.
...
public sealed class CardDetailPanel : MonoBehaviour
{
    ...
    private void AdoptHierarchy()
    {
        ...
        Transform panel = _canvas.transform.Find("Panel");
        if (panel == null) return;

        _panelRt = panel.GetComponent<RectTransform>();
        _panelCg = panel.GetComponent<CanvasGroup>();

        Transform titleT = panel.Find("Title");
        Transform rarityT = panel.Find("Rarity");
        Transform bodyT = panel.Find("Body");
