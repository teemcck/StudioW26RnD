...
// Instance must exist before Teleporter (default order) fades the overlay on arrival, which can happen in the scene's first frame.
...
public sealed class ChunkTransitionOverlay : MonoBehaviour
{
    ...
    private void AdoptHierarchy()
    {
        ...
        Transform fade = _canvas.transform.Find("BlackFade");
        if (fade == null) return;
