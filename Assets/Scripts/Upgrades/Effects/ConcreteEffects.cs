...
public class SpawnPoolModifierEffect : UpgradeEffect
{
    ...
    public override void Apply(UpgradeContext ctx)
    {
        foreach (string tag in addToPool) ctx.SpawnManager.AddEnemyType(tag);
        foreach (string tag in removeFromPool) ctx.SpawnManager.RemoveEnemyType(tag);
    }

    public override void Remove(UpgradeContext ctx)
    {
        foreach (string tag in removeFromPool) ctx.SpawnManager.AddEnemyType(tag);
        foreach (string tag in addToPool) ctx.SpawnManager.RemoveEnemyType(tag);
    }
...
{
    [Tooltip("Status effect applied to the player on each melee attack.")]
    public StatusEffectId statusEffectId = StatusEffectId.Swiftness;
