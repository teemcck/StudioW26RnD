...
public class ConditionalEffectRunner : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            ConditionalRecord rec      = _records[i];
    ...
    {
        List<UpgradeEffect> branch = state ? effect.whenTrue : effect.whenFalse;
        foreach (UpgradeEffect e in branch) e?.Apply(ctx);
    }

    private static void RemoveBranch(ConditionalEffect effect, UpgradeContext ctx, bool state)
    {
        List<UpgradeEffect> branch = state ? effect.whenTrue : effect.whenFalse;
        foreach (UpgradeEffect e in branch) e?.Remove(ctx);
    }
}
